...
describe('Learning Recommendations tenant isolation', () => {
  beforeEach(() => { vi.clearAllMocks(); });

  it('rejects cross-tenant event access', async () => {
    prismaMocks.commercialEvent.findFirst.mockResolvedValue(null);
    await expect(repo.generateRecommendations('tenant-a', 'event-from-tenant-b')).rejects.toThrow();
  });
  ...
  it('scopes event query to tenant', async () => {
    ...
    await repo.generateRecommendations('tenant-a', 'event-1');
  ...
  it('does not leak data across tenants', async () => {
    prismaMocks.commercialEvent.findFirst.mockResolvedValue(null);
    await expect(repo.generateRecommendations('tenant-b', 'event-from-tenant-a')).rejects.toThrow();
    expect(prismaMocks.commercialEvent.findFirst).toHaveBeenCalledWith(expect.objectContaining({

...
describe('historical assessment evidence repository', () => {
  beforeEach(() => {
    ...
    prismaMocks.commercialRevenueLine.findFirst.mockResolvedValue({
      id: scope.revenueLineId,
      name: 'Online Courses',
    });
  ...
  it('collects tenant-scoped, non-PII evidence and assigns event actuals to the linked explicit USD plan', async () => {
    ...
    expect(prismaMocks.commercialPlan.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          tenant_key: 'tenant-a',
          revenue_line_id: scope.revenueLineId,
        }),
      }),
    );
    expect(prismaMocks.eventKpiRecord.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          tenant_key: 'tenant-a',
          event_id: { in: ['33333333-3333-4333-8333-333333333333'] },
        }),
      }),
    );
    ...
    expect(preview.evidence).toContainEqual(
      expect.objectContaining({
        ...
        payload: expect.objectContaining({
          knownAmountPaid: 3000,
          knownOutstandingBalance: 2000,
          knownTicketQuantity: 2,
          paymentStatusCounts: { partial: 1 },
          ghlRecordCount: 1,
          attributedGhlRecordCount: 1,
  ...
  it('withholds mixed-currency event money instead of combining or relabeling it', () => {
      ...
      [
        {
          ...
          sourceObjectType: 'commercial_event',
          sourceObjectId: 'event-1',
          sourceName: 'Mixed plan event',
          metricKey: 'completed_event_context',
          metricValue: null,
          metricUnit: 'mixed',
          observedAt: new Date('2025-06-15'),
          payload: { currency: 'mixed' },
        },
        {
          evidenceType: 'event_kpi',
          sourceObjectType: 'event_channel_kpis',
          sourceObjectId: 'event-1:meta',
          sourceName: 'Mixed plan event - meta',
          metricKey: 'channel_performance',
          metricValue: 2500,
          metricUnit: 'mixed',
          observedAt: new Date('2025-06-16'),
          payload: {
            eventId: 'event-1',
            currency: 'mixed',
            channel: 'meta',
        ...
        },
        {
          ...
          metricValue: 3000,
          metricUnit: 'mixed',
          observedAt: new Date('2025-06-20'),
    ...
    expect(summary).toMatchObject({
      ...
      eventComparison: [
        expect.objectContaining({ currency: 'mixed', knownSpend: 0, knownRevenue: 0 }),
      ],
    });
  });
  ...
  it('builds a deterministic side-by-side comparison when two completed events have evidence', async () => {
    ...
    prismaMocks.commercialPlan.findMany.mockResolvedValue([
      ...
      },
      {
        ...
        title: 'Second leadership launch',
        status: 'completed',
        stage: 'implementation_engagement',
        horizon: 'product_or_event',
        objective: 'Sell leadership course',
        ...
        budget_target: 1500,
        revenue_target: 7000,
        strategy_summary: 'Buyer proof launch',
        action_plan: 'Content and follow-up',
        updated_at: new Date('2025-09-30'),
        linked_event_id: secondEventId,
        revenue_line: { name: 'Online Courses', revenue_line_type: 'online_course' },
      },
    ]);
    prismaMocks.commercialEvent.findMany.mockResolvedValue([
      ...
      {
        id: secondEventId,
        name: 'Leadership workshop - Autumn',
        event_type: 'virtual_event',
    ...
    prismaMocks.eventKpiRecord.findMany.mockResolvedValue([
      {
        ...
        source_type: 'connector',
        source_name: 'meta',
        metric_date: new Date('2025-06-16'),
        channel: 'instagram',
        reach: 10000,
        impressions: 15000,
        interactions: 1200,
      ...
      },
      {
        ...
        source_type: 'connector',
        source_name: 'meta',
        metric_date: new Date('2025-09-16'),
        channel: 'instagram',
        reach: 16000,
        impressions: 22000,
        interactions: 1800,
    ...
    expect(comparisons).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          eventName: 'Leadership workshop - Autumn',
          reach: 16000,
          leads: 150,
          purchases: 18,
          knownSpend: 3400,
  ...
  it('reports honest empty-state gaps without inventing evidence', async () => {
    ...
    expect(preview.evidence).toEqual([]);
    expect(preview.missingData).toContain(
      'No completed events were found in this assessment period.',
    );

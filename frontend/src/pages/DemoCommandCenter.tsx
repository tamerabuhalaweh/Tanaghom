  ...
  const setupTasks = [
    {
      ...
      meta: providerReady ? providerLabel : 'Add DeepSeek, OpenAI, or Claude so drafts are generated by a real provider.',
    ...
    async function run() {
      try {
        ...
        try {
          ...
          setProviderLabel(active ? `${text(active.name, 'LLM provider')} / ${text(active.model, 'model configured')}` : 'Connect DeepSeek, OpenAI, or Claude');
        } catch (providerError) {
    ...
    >
      <section className="rounded-2xl bg-[#0f0c1f] p-4 shadow-[0_24px_70px_rgba(15,15,22,0.30)] sm:p-5">
        ...
        <div className="mt-4">
          <ExecutiveStatusGrid items={[
            ...
            {
              label: 'AI model',
              value: providerReady ? providerLabel : 'Connect DeepSeek, OpenAI, or Claude',
      ...
      {!providerReady && (
        <Notice tone="warn">
          ...
          <Link to="/ai-settings" className="font-semibold underline">Connect DeepSeek, OpenAI, or Claude</Link>
        </Notice>
      )}
                ...
                <p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-neutral-600">
                  {selected
                    ? providerReady
                      ...
                      : 'Connect DeepSeek, OpenAI, or Claude for this user before generating real AI copy.'

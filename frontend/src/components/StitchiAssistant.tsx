...
function actionTitle(actionType: string): string {
  const labels: Record<string, string> = {
    ...
    create_commercial_plan: 'Create standalone execution exception',
    create_commercial_plan_with_revenue_line: 'Set up revenue line and create standalone exception',
    update_commercial_plan: 'Update execution plan',
    ...
    create_execution_plan_for_monthly_item: 'Create monthly execution plan',
...
export function StitchiChatPanel({ compact = false }: { compact?: boolean }) {
  ...
  const searchContext = useMemo(() => {
    ...
    return {
      ...
      weekStartDate: text(params.get('weekStartDate')),
  ...
  function requestPayload(content: string) {
    return {
      ...
      metadata: {
        ...
        ...(searchContext.weekStartDate ? { weekStartDate: searchContext.weekStartDate } : {}),

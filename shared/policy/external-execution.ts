...
export interface ExternalExecutionDecision {
  allowed: boolean;
  label: 'Working' | 'Sandbox Ready' | 'Blocked' | 'M5 Disabled';
  reasons: string[];
...
export function evaluateExternalExecution(input: ExternalExecutionInput): ExternalExecutionDecision {
  ...
  if (!isExternalWriteAction(input.action)) {
    return {
      allowed: true,
  ...
  const requiredFlags = ['EXTERNAL_EXECUTION_ENABLED', 'M5_WRITE_EXECUTION_ENABLED'];
  const systemFlag = SYSTEM_FLAGS[input.system];
  if (systemFlag) requiredFlags.push(systemFlag);

  if (!envEnabled('EXTERNAL_EXECUTION_ENABLED')) reasons.push('External execution kill switch is disabled');
  if (!envEnabled('M5_WRITE_EXECUTION_ENABLED')) reasons.push('M5 write execution is disabled');
  if (systemFlag && !envEnabled(systemFlag)) reasons.push(`${systemFlag} is disabled`);
  if (!input.humanApproved || !input.approvalId) reasons.push('Human approval is required');
  if (!input.capabilityResolutionId) reasons.push('Capability resolution is required');
  if (!input.mcpMediationRequestId) reasons.push('MCP mediation is required');
  if (executionMode === 'live' && process.env.DEMO_MODE === 'true') reasons.push('Live execution is blocked while DEMO_MODE=true');
